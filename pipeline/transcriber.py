...
class VADTranscriber:
    ...
    def detect_language(self) -> Tuple[str, float]:
        ...
        seg_gen, detect_info = self._whisper.transcribe(audio_data, beam_size=5)
        # 消费生成器
        for _ in seg_gen:
            pass
    ...
    def _transcribe_batch(self, seg_start: float, seg_end: float) -> list:
        """转录单个音频段，返回词列表"""
    ...
    def transcribe_all(self, language: Optional[str] = None,
        ...
        """
        # 模型加载
        model_load_time = self._load_model()
        ...
        for idx, (seg_start, seg_end) in enumerate(merged, 1):
            t0 = time.time()
            seg_words = self._transcribe_batch(seg_start, seg_end)
